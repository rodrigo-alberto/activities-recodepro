...
public class DestinyDao {
	public boolean createDestiny(Destiny destiny) {
		String sql = "INSERT INTO destiny (`name`, images, city) VALUES (?, ?, ?)";
	...
	public Destiny getDestinyById(int idDestiny) {
		...
		try {
			...
			while (rs.next()) {
				...
				outputDestiny.setName(rs.getString("name"));
	...
	public boolean updateDestiny(int idDestiny, Destiny destiny) {
		 String sql = "UPDATE destiny SET name = ?, images = ?, city = ? WHERE idDestiny = ?";
